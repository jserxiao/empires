...
function placeResources(map,cols,rows,seed){const rng=new PerlinNoise(seed+55555).seedRng(seed+66666),tN=new PerlinNoise(seed+11111),sN=new PerlinNoise(seed+22222),dN=new PerlinNoise(seed+33333)
  for(let y=0;y<rows;y++)for(let x=0;x<cols;x++){const tile=map[y][x],nx=x/cols,ny=y/rows
    ...
    else if(tile.terrain===TERRAIN.GRASS){const tv=tN.octaveNoise(nx*6,ny*6,3,.5);if(tv>0.4){const r=rng();if(r<0.25)tile.resource={key:'pine_tree',amount:RESOURCE_DEFS.pine_tree.amount};else if(r<0.35)tile.resource={key:'berry',amount:RESOURCE_DEFS.berry.amount}}else if(tv<0.05){const r=rng();if(r<0.04)tile.resource={key:'stone_cluster',amount:RESOURCE_DEFS.stone_cluster.amount};else if(r<0.07)tile.resource={key:'gold_cluster',amount:RESOURCE_DEFS.gold_cluster.amount}}}
    else if(tile.terrain===TERRAIN.FOREST){const tv=tN.octaveNoise(nx*8,ny*8,2,.5);if(tv>0.3){const r=rng();if(r<0.6)tile.resource={key:'pine_tree',amount:RESOURCE_DEFS.pine_tree.amount};else if(r<0.8)tile.resource={key:'pine_tree',amount:RESOURCE_DEFS.pine_tree.amount};else tile.resource={key:'berry',amount:RESOURCE_DEFS.berry.amount}}}
